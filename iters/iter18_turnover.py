"""iter18: Turnover threshold + DD lock 변형.

iter17 baseline: turnover<50% skip + DD-10% lock 63d
iter18: 다른 turnover threshold + DD/lock 조합 시도.
"""
...
def champ_wf(rets, cash_rets,
             turnover_thr=0.5, dd_stop=-0.10, lock_days=63,
             cvar_alpha=1.0,
             top_k=2,
...
def main():
    print("[iter18] Turnover threshold + DD lock variant sweep")
    ...
    print(f"\n=== Turnover threshold sweep (DD-10% lock 63d) ===")
    for t in [0.0, 0.2, 0.3, 0.5, 0.7, 1.0]:
        try:
            pnl = champ_wf(rets, cash, turnover_thr=t)
            m = metrics(pnl)
            color = "🚀" if m['Sharpe'] > 4.69 else "✅" if m['Sharpe'] > 4 else "⚠️" if m['Sharpe'] > 3 else "❌"
            print(f"  {color} turnover_thr={t}: Sharpe={m['Sharpe']:.2f} CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
        except Exception as e:
            print(f"  t={t}: ERROR {e}")

    print(f"\n=== DD lock variant sweep (turnover 0.5) ===")
    for dd, lock in [(-0.05, 30), (-0.05, 63), (-0.10, 30), (-0.10, 63), (-0.10, 90), (-0.10, 126), (-0.15, 63), (-0.15, 90), (-0.20, 90)]:
        try:
            pnl = champ_wf(rets, cash, dd_stop=dd, lock_days=lock)
            m = metrics(pnl)
            color = "🚀" if m['Sharpe'] > 4.69 and m['MDD'] > -0.16 else "✅" if m['Sharpe'] > 4 else "⚠️" if m['Sharpe'] > 3 else "❌"
            print(f"  {color} DD{dd*100:.0f}% lock{lock}d: Sharpe={m['Sharpe']:.2f} CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
        except Exception as e:
            print(f"  DD{dd} lock{lock}: ERROR {e}")

    print(f"\n  → results/iter18_turnover.json")
