...
def main():
    ...
    for t in [0.0, 0.2, 0.3, 0.5, 0.7, 1.0]:
        try:
            pnl, win_pnls = champ_wf(rets, cash, turnover_thr=t)
            ...
            print(f"  {color} turnover_thr={t}: Sharpe={m['mean_sharpe']:.2f} (win {nw-neg}/{nw}) CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
    ...
    for dd, lock in [(-0.05, 30), (-0.05, 63), (-0.10, 30), (-0.10, 63), (-0.10, 90), (-0.10, 126), (-0.15, 63), (-0.15, 90), (-0.20, 90)]:
        try:
            ...
            m = wf_metrics(pnl, win_pnls)
            neg = m['neg_windows']; nw = m['n_windows']
            color = "🚀" if m['mean_sharpe'] > 2.0 and m['MDD'] > -0.16 else "✅" if m['mean_sharpe'] > 1.0 else "⚠️" if m['mean_sharpe'] > 0.3 else "❌"
            print(f"  {color} DD{dd*100:.0f}% lock{lock}d: Sharpe={m['mean_sharpe']:.2f} (win {nw-neg}/{nw}) CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
        except Exception as e:
