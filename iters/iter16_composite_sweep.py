"""iter16: Composite period sweep (21+8 → 다른 조합).

iter04/14 baseline: periods=(21, 8), weights=(0.3, 0.7)
iter16: 다양한 period/weight 시도.
"""
...
def main():
    print("[iter16] Composite period sweep + k=2")
    ...
    print(f"\n=== Composite period × weight sweep (k=2 base) ===")
    configs = [
        ("(21,8) 0.3/0.7 [iter14]",       (21, 8),    (0.3, 0.7)),
        ("(21,8) 0.5/0.5",                 (21, 8),    (0.5, 0.5)),
        ("(21,8) 0.2/0.8",                 (21, 8),    (0.2, 0.8)),
        ("(21,8) 0.7/0.3 (long heavy)",    (21, 8),    (0.7, 0.3)),
        ("(63,21,8) 3-comp",               (63, 21, 8), (0.2, 0.3, 0.5)),
        ("(126,21,8) 3-comp long",         (126, 21, 8), (0.2, 0.3, 0.5)),
        ("(21,5)",                         (21, 5),    (0.3, 0.7)),
        ("(21,13)",                        (21, 13),   (0.3, 0.7)),
        ("(42,13,5) 3-comp short",         (42, 13, 5), (0.2, 0.4, 0.4)),
        ("8d only",                        (8,),       (1.0,)),
        ("21d only",                       (21,),      (1.0,)),
        ("(252,42,8) 3-comp very long",    (252, 42, 8), (0.2, 0.3, 0.5)),
    ]
    results = {}
    for name, p, w in configs:
        try:
            pnl = champ_wf_dd(rets, cash, periods=p, weights_p=w, top_k=2)
            m = metrics(pnl)
            results[name] = m
            color = "🚀" if m['Sharpe'] > 4.16 else "✅" if m['Sharpe'] > 3.5 else "⚠️" if m['Sharpe'] > 2 else "❌"
            print(f"  {color} {name}: Sharpe={m['Sharpe']:.2f} CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
        except Exception as e:
            print(f"  {name}: ERROR {e}")

    out_path = RESULTS_DIR / "iter16_composite_sweep.json"
