...
def main():
    ...
    for name, k, mode, cap, h in configs:
        try:
            pnl = champ_wf(rets, cash, top_k=k, weight_mode=mode, weight_cap=cap, hmm_high=h)
            m = metrics(pnl)
            results[name] = m
            mdd_ok = m['MDD'] > -0.30
            sharpe_ok = m['Sharpe'] > 1.5
            color = "🚀" if mdd_ok and m['Sharpe'] > 2 else \
    ...
    for fee_bps in [50, 100, 150]:
        pnl_c = champ_wf(rets, cash, top_k=3, weight_mode="equal", weight_cap=None,
                         hmm_high=0.6, fee_per_change=fee_bps/1e4)
        mc = metrics(pnl_c)
        cost_results[fee_bps] = mc
        color = "✅" if mc['Sharpe'] > 1.5 else "⚠️" if mc['Sharpe'] > 0.5 else "❌"
        print(f"  {color} {fee_bps}bps: Sharpe={mc['Sharpe']:.2f} MDD={mc['MDD']*100:.1f}%")

    # 종합
    best_name = max(results, key=lambda k: results[k].get('Sharpe', 0) if isinstance(results[k], dict) and 'error' not in results[k] else -999)
    best = results[best_name]
    print(f"\n=== iter02 종합 ===")
    print(f"  최고: {best_name}")
    print(f"  Sharpe={best.get('Sharpe', 0):.2f} CAGR={best.get('CAGR', 0)*100:.1f}% MDD={best.get('MDD', 0)*100:.1f}%")
