...
def main():
    ...
    for rebal in [1, 2, 3, 5, 10, 21]:
        try:
            pnl, win_pnls = champ_wf_dd(rets, cash, top_k=3, dd_stop=-0.10, lock_days=63, rebal_days=rebal)
            ...
            print(f"  {color} rebal={rebal}d: Sharpe={m['mean_sharpe']:.2f} (win {nw-neg}/{nw}) CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
