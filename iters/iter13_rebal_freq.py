"""iter13: Rebal frequency variation (2d → 5d/10d).

iter04 baseline: rebal 2d (turnover<50% skip).
iter13 가설: rebal 빈도 줄이면 거래비용 ↓ + Sharpe 유지 가능?

테스트: rebal 1d/2d/3d/5d/10d.
"""
        ...
        for i in range(len(test_idx)):
            ...
            if locked_until > current_idx:
                ...
                if len(lookback_pnl) > 30:
                    ...
                    w_eff = w
            if i > 0 and i % rebal_days == 0 and locked_until <= current_idx:
                end_pos = s + train + i
...
def main():
    print("[iter13] Rebal frequency sweep")
    ...
    print(f"\n=== Rebal frequency × DD-10% lock 63d ===")
    for rebal in [1, 2, 3, 5, 10, 21]:
        try:
            pnl = champ_wf_dd(rets, cash, top_k=3, dd_stop=-0.10, lock_days=63, rebal_days=rebal)
            m = metrics(pnl)
            color = "🚀" if m['Sharpe'] > 4 else "✅" if m['Sharpe'] > 3 else "⚠️" if m['Sharpe'] > 2 else "❌"
            print(f"  {color} rebal={rebal}d: Sharpe={m['Sharpe']:.2f} CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
        except Exception as e:
            print(f"  rebal={rebal}d: ERROR {e}")

    out_path = RESULTS_DIR / "iter13_rebal_freq.json"
