"""iter17: CVaR α + lookback sweep on iter16 (8d only + k=2).

iter16 baseline: 8d only + cvar_α=2.0, lookback=63
iter17: 다양한 cvar config 시도.
"""
...
def champ_wf(rets, cash_rets,
             cvar_alpha=2.0, cvar_lb=63, cvar_pct=5,
    ...
    def compute(end_idx):
        ...
        window = rets.iloc[end_idx - cvar_lb:end_idx]
        ...
        for col in window.columns:
            ...
            if len(ret) < 20:
                cvar_5[col] = 0.001
                continue
            threshold = np.percentile(ret, cvar_pct)
        ...
        scores = composite[composite > 0].dropna()  # strict_pos
        if scores.empty:
            return None
...
def main():
    print("[iter17] CVaR α/lookback sweep on iter16 (8d only + k=2)")
    ...
    print(f"\n=== CVaR config sweep ===")
    configs = [
        ("α=2.0 lb=63 5% [iter16]", 2.0, 63, 5),
        ("α=1.0 lb=63 5%",          1.0, 63, 5),
        ("α=1.5 lb=63 5%",          1.5, 63, 5),
        ("α=2.5 lb=63 5%",          2.5, 63, 5),
        ("α=3.0 lb=63 5%",          3.0, 63, 5),
        ("α=2.0 lb=42 5%",          2.0, 42, 5),
        ("α=2.0 lb=84 5%",          2.0, 84, 5),
        ("α=2.0 lb=126 5%",         2.0, 126, 5),
        ("α=2.0 lb=63 1%",          2.0, 63, 1),
        ("α=2.0 lb=63 10%",         2.0, 63, 10),
        ("α=2.0 lb=63 20%",         2.0, 63, 20),
        ("α=0.0 (CVaR off)",        0.0, 63, 5),
    ]
    results = {}
    for name, a, lb, pct in configs:
        try:
            pnl, win_pnls = champ_wf(rets, cash, cvar_alpha=a, cvar_lb=lb, cvar_pct=pct)
            ...
            neg = m['neg_windows']
            n = m['n_windows']
            print(f"  {color} {name}: Sharpe={sh:.2f} (win {n-neg}/{n}) CAGR={m['CAGR']*100:.1f}% MDD={m['MDD']*100:.1f}%")
        except Exception as e:
            print(f"  {name}: ERROR {e}")

    out_path = RESULTS_DIR / "iter17_cvar_sweep.json"
